/**
 * @fileoverview Shared error classification rules for cowork API errors.
 * Used by both renderer (UI) and main process (IM replies).
 *
 * This module provides a centralized way to classify various API errors
 * into standardized i18n keys for consistent error handling and messaging
 * across the application.
 */

/**
 * Array of error classification rules.
 * Each rule is a tuple of [RegExp pattern, i18n key].
 * Rules are evaluated in order - first match wins.
 */
const ERROR_RULES: Array<[RegExp, string]> = [
  // Auth errors: Anthropic, DeepSeek, OpenAI, Gemini authentication failures, HTTP 401
  ...
  // Rate limit errors: HTTP 429, Anthropic/DeepSeek overloaded, Gemini RESOURCE_EXHAUSTED
  // Note: Must precede billing rules so "RESOURCE_EXHAUSTED: quota exceeded" maps to rate-limit
  [/\b429\b|rate[_ ]limit|too many requests|overloaded|RESOURCE_EXHAUSTED/i, 'coworkErrorRateLimit'],

  // Billing errors: DeepSeek 402, OpenAI, OpenRouter, Qwen, StepFun insufficient balance
  ...
  // Input too long errors: context length exceeded, HTTP 413, payload too large
  ...
  // Model not found errors: standard, Qwen, Ollama
  ...
  // Content moderation errors: Qwen, StepFun 451, generic content filtering
...
/**
 * Classifies an error string and returns the matching i18n key.
 *
 * Iterates through ERROR_RULES in order and returns the key for the first
 * matching pattern. Returns null if no rule matches (caller should fall back
 * to displaying the original error message).
 *
 * @param error - The error message string to classify
 * @returns The i18n key for the classified error, or null if no match found
 *
 * @example
 * ```typescript
 * // Authentication error
 * classifyErrorKey('API key is invalid'); // 'coworkErrorAuthInvalid'
 *
 * // Rate limit error
 * classifyErrorKey('Rate limit exceeded: 429'); // 'coworkErrorRateLimit'
 *
 * // Unknown error - no match
 * classifyErrorKey('Some random error'); // null
 * ```
 */
