...
搜索音乐资源（歌曲、专辑、无损音乐）的网盘分享链接。

## 功能描述

Music Search Skill 通过实时爬虫深度抓取资源页面，从各网盘平台获取公开分享的音乐资源链接，支持 FLAC、APE、WAV、DSD、Hi-Res 等无损格式。
...
- `web` — 仅从 web-search 搜索引擎摘要中提取链接（速度快，但准确率较低）
...
- `format`: 检测到的音频格式 — `FLAC`, `APE`, `WAV`, `DSD`, `Hi-Res`, `AIFF`, `MP3`, `AAC`, `OGG`
